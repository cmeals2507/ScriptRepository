...
with open('CBDNASamples.csv', newline='') as database:
...
nextIndex = 0
nextArray = "CODE"
with open('https://www.dropbox.com/s/dn0sjo6p3euym17/2021Codebanks.txt?dl=0') as banks:
    for line in banks:
        case = 0;
        index = -1;
...
for row in dataList:
    if row[0] == "Semester": #skips the first row, which is column headers (before, it used to put numbers later in the first row, because of the else statements)
        continue
    if row[1] in code[3]:
        row.append("4")
...
with open("CBNDA Test.csv", "w+", newline="") as product:
